...
class SolveStep(PipelineStep):
    ...
    def run(self, ctx: PipelineContext):
        # 1. DATA после parse/apply_order
        DATA = ctx.data["DATA"]

        # 2. Создаём JSON для C++ решателя
        cpp_input = {
            "branches": DATA
        }

        tmp_json = os.path.join(ctx.output_dir, "astar_input.json")
        with open(tmp_json, "w") as f:
            json.dump(cpp_input, f)

        # 3. Путь к C++ решателю
        solver_exe = os.path.abspath("GroupProblem/True_Python/ctools/astar.exe")

        if not os.path.exists(solver_exe):
            raise RuntimeError(f"C++ solver not found: {solver_exe}")

        # 4. Вызываем C++ решатель
        print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - INFO - Running C++ solver…")
        ...
        if proc.returncode != 0:
            print(proc.stdout)
            print(proc.stderr)
            raise RuntimeError(f"C++ solver error (exit {proc.returncode})")

        # 5. JSON из stdout
        try:
            result = json.loads(proc.stdout)
        except Exception as e:
            print("===== RAW OUTPUT FROM C++ =====")
            print(proc.stdout)
            print("================================")
            raise RuntimeError(f"Invalid JSON from C++ solver: {e}")
        ...
        for i, row in enumerate(result_tree):
            print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - INFO - branch [{i+1:02d}] - {row}")

        # 7. Формируем JSON для сохранения
        payload = {
            "steps": steps_count,
            "moves": moves,
        ...
        ctx.data["DATA"] = result_tree  # новое состояние
